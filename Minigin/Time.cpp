...
dae::TimerKey dae::Time::AddTimer(const Timer& timer)
{
	++m_LastKey;
...
bool dae::Time::RemoveTimer(TimerKey key)
{
	return m_Timers.Erase(key);
}

dae::Timer& dae::Time::TimerAt(TimerKey key)
{
	return m_Timers.ValueAt(key);
}

bool dae::Time::IsTimerActive(TimerKey key) const
{
	return m_Timers.IsActivated(key);
}

void dae::Time::RestartTimer(TimerKey key)
{
	m_Timers.Activate(key);
	m_Timers.ValueAt(key).Restart();
}

void dae::Time::EndTimer(TimerKey key)
{
	m_Timers.Deactivate(key);
	m_Timers.ValueAt(key).End();
}

void dae::Time::PauseTimer(TimerKey key)
{
	m_Timers.Deactivate(key);
	m_Timers.ValueAt(key).Pause();
}

void dae::Time::PlayTimer(TimerKey key)
{
	m_Timers.Activate(key);
	m_Timers.ValueAt(key).Play();
}

void dae::Time::SetTimerTime(TimerKey key, float time)
{
...
void dae::Time::SetTimerDuration(TimerKey key, float duration, bool changeStateBasedOnDuration)
{
	Timer& timer = m_Timers.ValueAt(key);
...
dae::TimerKey dae::Time::TriggerFunctionAfterSeconds(const std::function<void()>& function, float seconds, bool removeTimerAfterTriggering, bool startTimerPaused)
{
	...
	{
		Time::GetInstance().TimerAt(key).GetOnEndEvent().Subscribe([key]()
			{

				Time::GetInstance().RemoveTimer(key);
...
dae::TimerKey dae::Time::StartEndFunctionWithDuration(const std::function<void()>& start, const std::function<void()>& end, float duration, bool removeTimerAfterTriggering, bool startTimerPaused)
{
...
size_t dae::Time::GetActivatedTimersSize() const
{
	return m_Timers.ActivatedSize();
}

void dae::Time::UpdateTimers()
{
