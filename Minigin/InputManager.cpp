...
void dae::Input::BindKey(const GamepadKeyData& gamepadKeyData)
{
	m_GamepadEvents.push_back(gamepadKeyData);
}

void dae::Input::BindKey(const KeyboardKeyData& keyboardKeyData)
{
...
void dae::InputManager::ProcessControllerInputs()
{
	...
	for (size_t i = 0; i < Input::GetInstance().m_GamepadEvents.size(); i++)
	{
		auto& gamepadEvent = Input::GetInstance().m_GamepadEvents[i];
		...
		case ButtonState::BUTTON_DOWN:
			if (IsDownThisFrame(gamepadEvent.GamepadButton, currentId))
				gamepadEvent.OnTriggered.Invoke();
			break;
...
void dae::InputManager::ProcessKeyboardInputs()
{
	...
	for (size_t i = 0; i < Input::GetInstance().m_KeyboardEvents.size(); i++)
	{
		auto& keyboardEvent = Input::GetInstance().m_KeyboardEvents[i];
		...
		if (triggered)
		{
			keyboardEvent.OnTriggered.Invoke();
		}
	}
}
