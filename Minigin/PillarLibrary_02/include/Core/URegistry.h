...
namespace Pillar {
	...
	*/
	...
	public:
		...
		URegistry(const URegistry& other) = delete;
		URegistry(URegistry&& other) = delete;
		URegistry& operator=(const URegistry& other) = delete;
		URegistry& operator=(URegistry&& other) = delete;
		~URegistry();
		...
		*/
		...
		template<typename ComponentType, typename ReturnType = void>
		...
		*/
		...
		void ActivateEntity(const FEntity& entity, bool removeFromMarkedForDeactivation = true);
		void DeactivateEntity(const FEntity& entity, bool removeFromMarkedForActivation = true);

		void MarkForDeactivation(const FEntity& entity, bool removeFromMarkedForActivation = true);
		void RemoveFromMarkedForDeactivation(const FEntity& entity);
		void DeactivateMarkedEntities();

		void MarkForActivation(const FEntity& entity, bool removeFromMarkedForDeactivation = true);
		void RemoveFromMarkedForActivation(const FEntity& entity);
		void ActivateMarkedEntities();
		...
		*/
		...
		/*
			LOCK ACTIVATION AND DEACTIVATION
		*/
		template<typename ComponentType>
	...
	private:
		...
		void Implementation_CreateEntity(FEntity& , bool ) {  }; 
		...
		void Implementation_CreateEntities(std::vector<FEntity>& , bool ) {  };
		...
		std::vector<FEntity> m_EntitiesMarkedForDeactivation;
		std::vector<FEntity> m_EntitiesMarkedForActivation;
	};
...
#pragma region Clear Components Of Type
	...
	template<typename ComponentType>
	inline void URegistry::DestroyComponentsOfType()
	{
		std::type_index id{ typeid(ComponentType) };
...
#pragma region Create Entities
	...
	inline void URegistry::DoTaskOverAllActivated(const std::function<void(ComponentType& component, ReturnType* returnValue)> function, ReturnType* outPut)
	{
		UComponentCompartment<ComponentType>& compartment = GetComponentCompartment<ComponentType>();

		compartment.DoTaskOverAllActivated(function, outPut);
	}
	template<typename ComponentType, typename ReturnType>
	inline void URegistry::DoTaskOverAllActivated(const std::function<void(ComponentType& component, const FEntity& entity, ReturnType* returnValue)> function, ReturnType* outPut)
	{
		UComponentCompartment<ComponentType>& compartment = GetComponentCompartment<ComponentType>();
	...
	template<typename ComponentType, typename ReturnType>
	inline void URegistry::DoCompareTaskOverAllActivated(const std::function<void(ComponentType& first, ComponentType& second, ReturnType* returnValue)> function, ReturnType* outPut)
	{
		UComponentCompartment<ComponentType>& compartment = GetComponentCompartment<ComponentType>();

		compartment.DoCompareTaskOverAllActivated(function, outPut);
	}
	template<typename ComponentType, typename ReturnType>
	inline void URegistry::DoCompareTaskOverAllActivated(const std::function<void(ComponentType& first, ComponentType& second, const FEntity& firstEntity, const FEntity& secondEntity, ReturnType* returnValue)> function, ReturnType* outPut)
	{
		UComponentCompartment<ComponentType>& compartment = GetComponentCompartment<ComponentType>();
...
#pragma region Add And Remove Component
	...
	inline const bool URegistry::AddComponent(FEntity& entity, const ComponentType& component)
	{
		...
		for (auto It = oldArchetype.begin(); It != oldArchetype.end(); ++It)
		{
			...
			m_TypeIndex_To_ComponentCompartment.at((*It)).get()->MoveComponent(entity, newEntity);
...
#pragma endregion
	...
	private:
		...
		void SetFilter(URegistry& registry) { registry; }
