...
namespace Pillar {
	...
	*/
	...
	/*
			If the registry is not locked, and you chnage the memory layout of the registry of this type
			it produces undefined behaviour
	*/
	template<typename ComponentType>
	class UComponentIterator final: public IComponentIterator{
	...
	public:
		...
		size_t NumOfArchetypes() const;
		DArchetypeId GetArchetype(size_t archetypeIndex) const;

		size_t TotalNumOfComponents() const;
		size_t NumOfComponents(size_t archetypeIndex) const;
		size_t NumOfActivatedComponents(size_t archetypeIndex) const;

		std::vector<ComponentType>&& Move(DArchetypeId id);
		std::vector<ComponentType>&& Move(size_t index);
		void MoveBack(DArchetypeId id, std::vector<ComponentType>&& vector);
		void MoveBack(size_t index, std::vector<ComponentType>&& vector);
...
#pragma region Num Of
	template<typename ComponentType>
	inline size_t UComponentIterator<ComponentType>::NumOfArchetypes() const
	{
		return m_ComponentArrayPtrs.size();
	}

	template<typename ComponentType>
	inline DArchetypeId UComponentIterator<ComponentType>::GetArchetype(size_t archetypeIndex) const
	{
		return m_ArchetypeIds.at(archetypeIndex);
	}

	template<typename ComponentType>
	inline size_t UComponentIterator<ComponentType>::TotalNumOfComponents() const
	{
		size_t count{0};
	...
	template<typename ComponentType>
	inline size_t UComponentIterator<ComponentType>::NumOfComponents(size_t archetypeIndex) const
	{
	...
	template<typename ComponentType>
	inline size_t UComponentIterator<ComponentType>::NumOfActivatedComponents(size_t archetypeIndex) const
	{
		if (m_ComponentCompartment && archetypeIndex < m_ArchetypeIds.size())
		{
	...
	template<typename ComponentType>
	inline std::vector<ComponentType>&& UComponentIterator<ComponentType>::Move(DArchetypeId id)
	{
		return std::move(*DataAt(id));
	}

	template<typename ComponentType>
	inline std::vector<ComponentType>&& UComponentIterator<ComponentType>::Move(size_t index)
	{
		return std::move(*m_ComponentArrayPtrs[index]);
	}

	template<typename ComponentType>
	inline void UComponentIterator<ComponentType>::MoveBack(DArchetypeId id, std::vector<ComponentType>&& vector)
	{
		*DataAt(id) = vector;
	}

	template<typename ComponentType>
	inline void UComponentIterator<ComponentType>::MoveBack(size_t index, std::vector<ComponentType>&& vector)
	{
		*m_ComponentArrayPtrs[index] = vector;
	}
