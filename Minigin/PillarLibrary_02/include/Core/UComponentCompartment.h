...
namespace Pillar {
	...
	public:
		...
		virtual bool IsActivated(const FEntity& entity) = 0;

		virtual bool Create(const FEntity& entity) = 0;
		virtual bool Destroy(const FEntity& entity) = 0;

		virtual size_t TotalSizeOfActivated() = 0;
		virtual size_t SizeOfActivated(DArchetypeId archetype) = 0;
		virtual size_t Size(DArchetypeId archetype) = 0;

		virtual DEntityId GetEntity(size_t componentIndex, DArchetypeId archetype) = 0;
		virtual void MoveComponent(const FEntity& from, const FEntity& to) = 0;
	...
	class FArchetypeSearchFilter {


	};

	template<typename ComponentType>
	class UComponentCompartment final: public IComponentCompartment{
	public:
		virtual void ActivateAll() override;
		virtual void DeactivateAll() override;

		virtual void Activate(const FEntity& entity) override;
		virtual void Deactivate(const FEntity& entity) override;
		virtual bool IsActivated(const FEntity& entity) override;

		virtual bool Create(const FEntity& entity) override;
		virtual bool Destroy(const FEntity& entity) override;

		template<typename ReturnType = void>
		void DoTaskOverAllActivated(const std::function<void(ComponentType& component, ReturnType* returnValue)> function, ReturnType* outPut = nullptr);

		template<typename ReturnType = void>
		void DoTaskOverAllActivated(const std::function<void(ComponentType& component, const FEntity& entity, ReturnType* returnValue)> function, ReturnType* outPut = nullptr);

		template<typename ReturnType = void>
		void DoCompareTaskOverAllActivated(const std::function<void(ComponentType& first, ComponentType& second, ReturnType* returnValue)> function, ReturnType* outPut = nullptr);
		template<typename ReturnType = void>
		void DoCompareTaskOverAllActivated(const std::function<void(ComponentType& first, ComponentType& second, const FEntity& firstEntity, const FEntity& secondEntity, ReturnType* returnValue)> function, ReturnType* outPut = nullptr);

		virtual size_t TotalSizeOfActivated() override;
		virtual size_t SizeOfActivated(DArchetypeId archetype) override;
		size_t Size(DArchetypeId archetype);

		virtual DEntityId GetEntity(size_t componentIndex, DArchetypeId archetype) override;
		virtual void MoveComponent(const FEntity& from, const FEntity& to) override;
		...
		bool Create(const FEntity& entity, const ComponentType& component); 
	...
	private:
		...
		std::vector<std::vector<FEntity>>		m_ArchetypeEntities;
...
#pragma region Activation
		...
		if (m_ArchetypeId_To_ComponentIndexInformation.contains(entity.Archetype_Id))
		{
			if (m_ArchetypeId_To_ComponentIndexInformation.at(entity.Archetype_Id).EntityId_To_ComponentIndex.contains(entity.Entity_Id))
			{
				...
				else if (componentIndex > sizeOfActivated)
				{
					...
					FEntity tempEntity	= m_ArchetypeEntities[archetypeIndex][sizeOfActivated];
					...
					m_ArchetypeId_To_ComponentIndexInformation.at(entity.Archetype_Id).EntityId_To_ComponentIndex.at(tempEntity.Entity_Id) = componentIndex;
		...
		if (m_ArchetypeId_To_ComponentIndexInformation.contains(entity.Archetype_Id))
		{
			if (m_ArchetypeId_To_ComponentIndexInformation.at(entity.Archetype_Id).EntityId_To_ComponentIndex.contains(entity.Entity_Id))
			{
				...
				else if (componentIndex < sizeOfActivated)
				{
					...
					FEntity tempEntity = m_ArchetypeEntities[archetypeIndex][sizeOfActivated - 1];
	...
	template<typename ComponentType>
	inline bool UComponentCompartment<ComponentType>::IsActivated(const FEntity& entity)
	{
		if (m_ArchetypeId_To_ComponentIndexInformation.contains(entity.Archetype_Id))
		{
			if (m_ArchetypeId_To_ComponentIndexInformation.at(entity.Archetype_Id).EntityId_To_ComponentIndex.contains(entity.Entity_Id))
...
#pragma region Get Entity
	...
	inline DEntityId UComponentCompartment<ComponentType>::GetEntity(size_t componentIndex, DArchetypeId archetype)
	{
		if (m_ArchetypeId_To_ComponentIndexInformation.contains(archetype))
		{
			...
			return m_ArchetypeEntities[matrixIndex][componentIndex].Entity_Id;
		}
...
#pragma region Move Entity
	...
	template<typename ComponentType>
	inline void UComponentCompartment<ComponentType>::MoveComponent(const FEntity& from, const FEntity& to)
	{
...
#pragma region Create
	...
	inline bool UComponentCompartment<ComponentType>::Create(const FEntity& entity, const ComponentType& component)
	{
		...
		m_ArchetypeEntities[matrixIndex].emplace_back(entity);
		Activate(entity);
...
#pragma region Destroy
	...
	template<typename ComponentType>
	inline bool UComponentCompartment<ComponentType>::Create(const FEntity& entity)
	{
		if (!m_DefaultComponent)
		{
			return false;
	...
	template<typename ComponentType>
	inline bool UComponentCompartment<ComponentType>::Destroy(const FEntity& entity)
	{
		...
		if (m_ArchetypeId_To_ComponentIndexInformation.contains(entity.Archetype_Id))
		{
			if (m_ArchetypeId_To_ComponentIndexInformation.at(entity.Archetype_Id).EntityId_To_ComponentIndex.contains(entity.Entity_Id))
			{
				...
				std::vector<FEntity>& entityVector = m_ArchetypeEntities[matrixIndex];
				...
				m_ArchetypeId_To_ComponentIndexInformation.at(entity.Archetype_Id).EntityId_To_ComponentIndex.at(entityVector[componentIndex].Entity_Id) = componentIndex;
	...
	template<typename ComponentType>
	inline size_t UComponentCompartment<ComponentType>::TotalSizeOfActivated()
	{
		size_t counter{};
	...
	template<typename ComponentType>
	inline size_t UComponentCompartment<ComponentType>::SizeOfActivated(DArchetypeId archetype)
	{
	...
	template<typename ComponentType>
	inline size_t UComponentCompartment<ComponentType>::Size(DArchetypeId archetype)
	{
		if (m_ArchetypeId_To_ComponentIndexInformation.contains(archetype))
		{
...
#pragma endregion
	...
	inline void UComponentCompartment<ComponentType>::DoTaskOverAllActivated(const std::function<void(ComponentType&, ReturnType* returnValue)> function, ReturnType* outPut)
	{

		for (size_t archIdx = 0; archIdx < m_ArchetypeComponents.size(); archIdx++)
		{
			for (size_t compIdx = 0; compIdx < m_SizeOfActivated[archIdx]; compIdx++)
			{
				function(m_ArchetypeComponents[archIdx][compIdx], outPut);
			}
		}
	}
	...
	template<typename ComponentType>
	template<typename ReturnType>
	inline void UComponentCompartment<ComponentType>::DoTaskOverAllActivated(const std::function<void(ComponentType& component, const FEntity& entity, ReturnType* returnValue)> function, ReturnType* outPut) {
		...
		for (size_t archIdx = 0; archIdx < m_ArchetypeComponents.size(); archIdx++)
		{
			for (size_t compIdx = 0; compIdx < m_SizeOfActivated[archIdx]; compIdx++)
			{
				...
				function(m_ArchetypeComponents[archIdx][compIdx], m_ArchetypeEntities[archIdx][compIdx], outPut);
			}
		}
	}
	...
	template<typename ComponentType>
	template<typename ReturnType>
	inline void UComponentCompartment<ComponentType>::DoCompareTaskOverAllActivated(const std::function<void(ComponentType& first, ComponentType& second, ReturnType* returnValue)> function, ReturnType* outPut)
	{
		for (size_t archIdx = 0; archIdx < m_ArchetypeComponents.size(); archIdx++)
		{
			for (size_t compIdx = 0; compIdx < m_SizeOfActivated[archIdx]; compIdx++)
			{
				...
				for (size_t otherArchIdx = archIdx; otherArchIdx < m_ArchetypeComponents.size(); otherArchIdx++)
				{
					for (size_t otherCompIdx = 0; otherCompIdx < m_SizeOfActivated[otherArchIdx]; otherCompIdx++)
					{
						...
						function(
							m_ArchetypeComponents[archIdx][compIdx],
							m_ArchetypeComponents[otherArchIdx][otherCompIdx],
							outPut);
					}
				}
	...
	template<typename ComponentType>
	template<typename ReturnType>
	inline void UComponentCompartment<ComponentType>::DoCompareTaskOverAllActivated(const std::function<void(ComponentType& first, ComponentType& second, const FEntity& firstEntity, const FEntity& secondEntity, ReturnType* returnValue)> function, ReturnType* outPut) {
				...
				for (size_t otherArchIdx = archIdx; otherArchIdx < m_ArchetypeComponents.size(); otherArchIdx++)
				{
					for (size_t otherCompIdx = 0; otherCompIdx < m_SizeOfActivated[otherArchIdx]; otherCompIdx++)
					{
						...
						function(
							m_ArchetypeComponents[archIdx][compIdx],
							m_ArchetypeComponents[otherArchIdx][otherCompIdx],
							m_ArchetypeEntities[archIdx][compIdx],
							m_ArchetypeEntities[otherArchIdx][otherCompIdx],
							outPut);
					}
				}
