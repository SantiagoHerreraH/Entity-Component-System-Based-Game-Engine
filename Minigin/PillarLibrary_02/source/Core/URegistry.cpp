...
#pragma region Activation and Deactivation
	...
	void URegistry::ActivateEntity(const FEntity& entity, bool removeFromMarkedForDeactivation)
	{
		if (!Exists(entity) || IsActivated(entity))
		{
			return;
		}
		if (removeFromMarkedForDeactivation)
		{
			RemoveFromMarkedForActivation(entity);
		}
	...
	void URegistry::DeactivateEntity(const FEntity& entity, bool removeFromMarkedForActivation)
	{
		if (!Exists(entity) || !IsActivated(entity))
		{
			return;
		...
		if (removeFromMarkedForActivation)
		{
			RemoveFromMarkedForActivation(entity);
		}
	...
	void URegistry::MarkForDeactivation(const FEntity& entity, bool removeFromMarkedForActivation )
	{
		m_EntitiesMarkedForDeactivation.push_back(entity);
	...
	void URegistry::RemoveFromMarkedForDeactivation(const FEntity& entity)
	{
		for (int i = 0; i < m_EntitiesMarkedForDeactivation.size(); i++)
		{
			if (m_EntitiesMarkedForDeactivation[i] == entity)
			{
				m_EntitiesMarkedForDeactivation.erase(m_EntitiesMarkedForDeactivation.begin() + i);
				--i;
			}
		}
	}

	void URegistry::DeactivateMarkedEntities()
	{
		for (int i = 0; i < m_EntitiesMarkedForDeactivation.size(); i++)
		{
			DeactivateEntity(m_EntitiesMarkedForDeactivation[i]);
		}

		m_EntitiesMarkedForDeactivation.clear();
	}

	void URegistry::MarkForActivation(const FEntity& entity, bool removeFromMarkedForDeactivation)
	{
		m_EntitiesMarkedForActivation.push_back(entity);

		if (removeFromMarkedForDeactivation)
		{
			RemoveFromMarkedForDeactivation(entity);
		}
	}

	void URegistry::RemoveFromMarkedForActivation(const FEntity& entity)
	{
		for (int i = 0; i < m_EntitiesMarkedForActivation.size(); i++)
		{
			if (m_EntitiesMarkedForActivation[i] == entity)
			{
				m_EntitiesMarkedForActivation.erase(m_EntitiesMarkedForActivation.begin() + i);
				--i;
			}
		}
	}

	void URegistry::ActivateMarkedEntities()
	{
		for (size_t i = 0; i < m_EntitiesMarkedForActivation.size(); i++)
		{
			ActivateEntity(m_EntitiesMarkedForActivation[i]);
		}

		m_EntitiesMarkedForActivation.clear();
	}
